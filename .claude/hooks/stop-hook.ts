...
try {
  const voicesPath = join(PAI_DIR, 'voice-server/voices.json');
...
async function main() {
  ...
  if (tabTitle) {
    try {
      ...
      // Use process.stderr.write to set tab title (cross-platform)
      // Works in Windows Terminal, Kitty, iTerm2, etc.
