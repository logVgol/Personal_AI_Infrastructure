...
async function main() {
  try {
    ...
    // Check if this is from a subagent based on cwd containing 'agents'
    else if (hookData.cwd && (hookData.cwd.includes('/agents/') || hookData.cwd.includes('\\agents\\'))) {
      // Extract agent name from path like "/agents/designer/" or "\agents\designer\"
